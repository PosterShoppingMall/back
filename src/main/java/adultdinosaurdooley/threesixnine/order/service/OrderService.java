...
public class OrderService {
    ...
    public SellProductDto ordersList(Long userId, int page, int size) {
        ...
        Pageable pageable = PageRequest.of(page,size);
         ...
         // 주문 상세 내역 페이지 가져오기
        Page<OrderDetailEntity> orderDetailPage = orderDetailRepository.findAllByOrdersId(validateUser.getId(), pageable);
        ...
        List<SellProductDto.ResponseOrderProduct> orderProductList = orderDetailPage.getContent().stream()
                .map(orderDetail -> {
                    // 상품 이미지 리스트 가져오기
                    List<ProductImageEntity> productImages = getProductImagesForProduct(orderDetail.getProduct().getId());
